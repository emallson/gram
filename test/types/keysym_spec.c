...
START_TEST(test_keysym_equalp_diff_leds)
{
  ...
  /* LEDs are currently ignored as they have no representation in Guile */
  ck_assert(SCM_BOOL_T == scm_equal_p(M_x_scm, M_x_scm2));
}
END_TEST
...
START_TEST(test_keysym_display)
{
  ...
  ck_assert_str_eq(scm_to_locale_string( scm_get_output_string (port)), "#<keysym M-x>");
  scm_close(port);
  ...
  scm_display(M_x_scm, port);
  ck_assert_str_eq(scm_to_locale_string( scm_get_output_string (port)), "#<keysym S-C-M-x>");
  scm_close(port);
  ...
  scm_display(M_x_scm, port);
  ck_assert_str_eq(scm_to_locale_string( scm_get_output_string (port)), "#<keysym BackSpace>");
  scm_close(port);
...
START_TEST(test_keysym_display_unicode)
{
  ...
  scm_display(M_x_scm, port);
  ck_assert_str_eq(scm_to_utf8_string ( scm_get_output_string (port)), "#<keysym ü>");
  scm_close(port);
  ...
  ck_assert_uint_eq(0, xkb_keysym_from_name("💩", XKB_KEYSYM_CASE_INSENSITIVE));
}
END_TEST

Suite* keysym_suite(void)
{
  Suite *s;
  TCase *tc_core, *tc_convert, *tc_equalp, *tc_display;

  s = suite_create("keysym");
